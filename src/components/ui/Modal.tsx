...
export const Modal: React.FC<ModalProps> = ({
  ...
  useEffect(() => {
    ...
    const focusables = () => dialogRef.current ? Array.from(dialogRef.current.querySelectorAll<HTMLElement>('button:not([disabled]),input:not([disabled]),textarea:not([disabled]),select:not([disabled]),[href],[tabindex="0"]')).filter(el => el.getClientRects().length) : [];
