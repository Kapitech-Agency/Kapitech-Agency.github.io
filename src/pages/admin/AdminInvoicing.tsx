...
export const AdminInvoicing: React.FC = () => {
  ...
  if (!canViewFinancials) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-8 bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] rounded-2xl max-w-xl mx-auto my-12 animate-in fade-in duration-200">
        <div className="w-16 h-16 rounded-2xl bg-red-500/10 border border-red-500/20 text-[var(--k-red-hover)] flex items-center justify-center mb-4 shadow-lg shadow-red-500/5">
          <ShieldCheck size={32} />
        </div>
        ...
        <div className="px-4 py-2.5 rounded-xl bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] text-xs font-mono text-[var(--k-text-secondary)]">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[rgba(255,255,255,0.07)]">
        <div>
          <h1 className="text-2xl font-display font-bold text-white flex items-center gap-3">
            <Receipt className="text-[var(--k-red-hover)]" size={26} />
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            className="h-10 px-4 rounded-xl bg-[var(--k-surface-raised)] hover:bg-[var(--k-surface-subtle)] text-white text-xs font-mono font-bold border border-[rgba(255,255,255,0.07)] transition-all flex items-center justify-center gap-1.5 min-h-[40px] disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Plus size={14} />
          ...
          {canManageInvoices && (
            <button
              ...
              className="h-10 px-4 rounded-xl bg-[var(--k-red)] hover:bg-[var(--k-red-hover)] text-white text-xs font-mono font-bold transition-all flex items-center justify-center gap-1.5 shadow-lg shadow-[var(--k-red)]/20 min-h-[40px]"
            >
              <Plus size={14} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] p-5 rounded-2xl flex flex-col justify-between h-full group hover:border-[#484F58] transition-all">
          <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] p-3 sm:p-4 rounded-2xl">
        
        {/* Left: Tab Switcher */}
        <div className="flex items-center gap-1.5 bg-[var(--k-surface-raised)] p-1 rounded-xl border border-[rgba(255,255,255,0.07)] shrink-0">
          <button
          ...
          <div className="flex flex-wrap items-center gap-2 flex-1 sm:justify-end">
            <div className="relative flex-1 max-w-xs">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] rounded-xl text-xs text-white placeholder-[var(--k-text-muted)] focus:outline-none focus:border-[var(--k-red)] font-mono"
              />
            </div>
      ...
      {activeTab === 'invoices' ? (
        <>
          ...
          <div className="md:hidden space-y-3">
            {filteredInvoices.length === 0 ? (
              <div className="p-8 text-center bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] rounded-2xl text-xs font-mono text-[var(--k-text-secondary)]">
              ...
              filteredInvoices.map((inv) => (
                <div 
                  key={inv.id}
                  className="bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] hover:border-[rgba(255,255,255,0.15)] rounded-2xl p-4 space-y-3.5 transition-all shadow-lg"
                >
                  {/* Card Header: Invoice # & Status */}
                  <div className="flex items-center justify-between gap-2">
                    <button 
                      onClick={() => setPreviewInvoice(inv)} 
                      className="font-bold text-white font-display text-sm hover:text-[var(--k-red-hover)] flex items-center gap-1.5 transition-colors"
                    >
                  ...
                  {/* Client & Dates */}
                  <div className="bg-[var(--k-surface-raised)]/60 rounded-xl p-3 border border-[rgba(255,255,255,0.04)] space-y-1.5 text-xs font-mono">
                    ...
                    <div className="flex items-center justify-between pt-1 border-t border-[rgba(255,255,255,0.05)] text-[11px]">
                      <span className="text-[var(--k-text-muted)]">Issue: {inv.issueDate}</span>
                      <span className="text-[var(--k-red-hover)] font-semibold">Due: {inv.dueDate}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <div>
                      <div className="text-[10px] uppercase font-mono text-[var(--k-text-secondary)]">{language === 'id' ? 'Total Tagihan' : 'Total Amount'}</div>
                      <div className="text-base font-bold text-emerald-400 font-display">
                        {formatAmount(inv.total, currency)}
                      </div>
                      <div className="text-[10px] font-mono text-[var(--k-text-muted)]">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        onClick={() => setPreviewInvoice(inv)}
                        className="h-9 px-3 rounded-xl bg-[var(--k-surface-raised)] hover:bg-[var(--k-surface-subtle)] text-[var(--k-text-secondary)] hover:text-white border border-[rgba(255,255,255,0.07)] text-xs font-mono flex items-center justify-center gap-1 transition-colors min-h-[36px]"
                      ...
                      <button
                        ...
                        className="w-9 h-9 rounded-xl bg-[var(--k-surface-raised)] hover:bg-[var(--k-surface-subtle)] text-[var(--k-text-secondary)] hover:text-white border border-[rgba(255,255,255,0.07)] flex items-center justify-center transition-colors min-h-[36px] min-w-[36px]"
                        title="Edit Invoice"
                      >
                        <Edit3 size={14} />
                      </button>
                      {canDeleteInvoice && (
                        <button
                          ...
                          className="w-9 h-9 rounded-xl bg-[var(--k-surface-raised)] hover:bg-red-950/40 text-[var(--k-text-secondary)] hover:text-red-400 border border-[rgba(255,255,255,0.07)] hover:border-red-500/30 flex items-center justify-center transition-colors min-h-[36px] min-w-[36px]"
          ...
          <ScrollShadowContainer
            ...
            scrollClassName="bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] rounded-2xl overflow-x-auto shadow-xl select-none"
          >
                  ...
                  filteredInvoices.map((inv) => (
                    <tr key={inv.id} className="hover:bg-[#1C2128] transition-colors group">
                      <td className="py-3.5 px-4 font-bold text-white font-display">
                        <button 
                          onClick={() => setPreviewInvoice(inv)} 
                          className="hover:text-[var(--k-red-hover)] flex items-center gap-1.5"
                        >
                          <span>{inv.invoiceNumber}</span>
                          <ExternalLink size={11} className="text-[var(--k-text-secondary)] group-hover:text-[var(--k-red-hover)]" />
                        </button>
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-[var(--k-text-secondary)]">
                        ...
                        <div className="text-[10px] text-[var(--k-text-muted)]">Due: {inv.dueDate}</div>
                      </td>
                      <td className="py-3.5 px-4 font-bold text-emerald-400 font-display">
                        {formatAmount(inv.total, currency)}
                        <div className="text-[10px] font-mono text-[var(--k-text-muted)] font-normal">
                          incl. {inv.taxPercent}% PPN
                        </div>
                        {((inv.amountPaid && inv.amountPaid > 0) || inv.status === 'partially_paid') && (
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {expenses.length === 0 ? (
              ...
              expenses.map((exp) => (
                <div 
                  key={exp.id}
                  className="bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-4 space-y-3 shadow-lg"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-mono text-[var(--k-text-secondary)]">{exp.date}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-[rgba(255,255,255,0.05)]">
                    <div>
                      <div className="text-[10px] font-mono text-[var(--k-text-secondary)]">{language === 'id' ? 'Nominal Pengeluaran' : 'Expense Amount'}</div>
          ...
          <div 
            ref={tableScrollRef}
            className="hidden md:block bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] rounded-2xl overflow-x-auto shadow-xl select-none"
          >
                ...
                {expenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-[#1C2128] transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        onClick={() => handleDeleteExpense(exp.id)}
                        className="w-9 h-9 rounded-xl bg-[var(--k-surface-raised)] hover:bg-red-950/40 text-[var(--k-text-secondary)] hover:text-red-400 border border-[rgba(255,255,255,0.07)] hover:border-red-500/30 inline-flex items-center justify-center transition-colors min-h-[36px] min-w-[36px]"
                        title="Delete Record"
                      >
                        <Trash2 size={14} />
                      </button>
      ...
      {isInvoiceModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-0 sm:p-4 overflow-y-auto">
          <div className="bg-[var(--k-surface-raised)] border-0 sm:border sm:border-[rgba(255,255,255,0.07)] rounded-none sm:rounded-2xl w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-2xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="sticky top-0 z-20 bg-[var(--k-surface-raised)]/95 backdrop-blur-md px-5 sm:px-6 py-4 border-b border-[rgba(255,255,255,0.07)] flex items-center justify-between shrink-0">
              <h3 className="font-display font-bold text-white text-base sm:text-lg flex items-center gap-2">
                <Receipt className="text-[var(--k-red-hover)]" size={20} />
                <span>{editingInvoice ? 'Edit Client Invoice' : 'Create New Invoice'}</span>
              </h3>
              <button 
                onClick={() => setIsInvoiceModalOpen(false)} 
                className="w-8 h-8 rounded-lg text-[var(--k-text-secondary)] hover:text-white bg-[var(--k-bg)] border border-[rgba(255,255,255,0.07)] flex items-center justify-center transition-colors shrink-0 ml-3"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveInvoice} className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-4 text-xs font-mono custom-scrollbar">
                {!editingInvoice && availableProjects.length > 0 && (
                  <div className="p-3 bg-[var(--k-bg)] border border-[rgba(255,255,255,0.07)] rounded-xl space-y-1.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2.5 bg-[var(--k-bg)] border border-[rgba(255,255,255,0.07)] rounded-xl text-white focus:outline-none focus:border-[var(--k-red)]"
                    />
                  </div>
                  <div>
                ...
                <div className="p-4 bg-[var(--k-bg)] border border-[rgba(255,255,255,0.07)] rounded-xl space-y-3">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-3 py-2 bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] rounded-xl text-white focus:outline-none focus:border-[var(--k-red)]"
                    />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2.5 bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] rounded-xl text-white focus:outline-none focus:border-[var(--k-red)]"
                      />
                    </div>
                    <div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 bg-[var(--k-bg)] border border-[rgba(255,255,255,0.07)] rounded-xl text-white focus:outline-none focus:border-[var(--k-red)]"
                  />
                </div>
              </div>
              ...
              <div className="sticky bottom-0 z-20 bg-[var(--k-surface-raised)]/95 backdrop-blur-md px-5 sm:px-6 py-3.5 border-t border-[rgba(255,255,255,0.07)] flex items-center justify-end gap-2.5 shrink-0">
                <button
                  type="button"
                  onClick={() => setIsInvoiceModalOpen(false)}
                  className="h-10 px-4 rounded-xl bg-[var(--k-bg)] hover:bg-[var(--k-surface-raised)] text-[var(--k-text-secondary)] hover:text-white border border-[rgba(255,255,255,0.07)] text-xs font-mono font-medium transition-colors min-h-[40px]"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="h-10 px-5 rounded-xl bg-[var(--k-red)] hover:bg-[var(--k-red-hover)] text-white text-xs font-mono font-bold transition-all shadow-lg shadow-[var(--k-red)]/20 min-h-[40px]"
                >
                  Save Invoice
                </button>
              </div>
      ...
      {isExpenseModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-0 sm:p-4 overflow-y-auto">
          <div className="bg-[var(--k-surface-raised)] border-0 sm:border sm:border-[rgba(255,255,255,0.07)] rounded-none sm:rounded-2xl w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-md shadow-2xl flex flex-col overflow-hidden">
            
            {/* Sticky Header */}
            <div className="sticky top-0 z-20 bg-[var(--k-surface-raised)]/95 backdrop-blur-md px-5 sm:px-6 py-4 border-b border-[rgba(255,255,255,0.07)] flex items-center justify-between shrink-0">
              <h3 className="font-display font-bold text-white text-base flex items-center gap-2">
                <CreditCard className="text-[var(--k-red-hover)]" size={18} />
      ...
      {paymentModalInvoice && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="bg-[var(--k-surface)] border border-[rgba(255,255,255,0.1)] rounded-2xl w-full max-w-lg shadow-2xl relative overflow-hidden flex flex-col max-h-[92vh]">
            {/* Modal Header */}
            <div className="sticky top-0 z-20 bg-[var(--k-surface-raised)]/95 backdrop-blur-md px-5 sm:px-6 py-4 border-b border-[rgba(255,255,255,0.07)] flex items-center justify-between shrink-0">
              ...
              <button
                ...
                className="w-8 h-8 rounded-xl bg-[var(--k-surface)] hover:bg-[var(--k-surface-subtle)] text-[var(--k-text-secondary)] hover:text-white flex items-center justify-center transition-colors border border-[rgba(255,255,255,0.07)]"
              >
                <X size={15} />
              </button>
            </div>
            ...
            <form onSubmit={handleRecordPaymentSubmit} className="flex flex-col flex-1 overflow-y-auto">
              <div className="p-5 sm:p-6 space-y-4 text-xs font-mono">
                {/* Summary Box */}
                <div className="bg-[var(--k-surface-raised)] p-4 rounded-xl border border-[rgba(255,255,255,0.06)] space-y-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.08)] rounded-xl text-white text-sm focus:outline-none focus:border-emerald-500 font-bold font-mono"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full px-3 py-2.5 bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.08)] rounded-xl text-white focus:outline-none focus:border-emerald-500 font-mono"
                    />
                  </div>
                </div>
              ...
              <div className="sticky bottom-0 z-20 bg-[var(--k-surface-raised)]/95 backdrop-blur-md px-5 sm:px-6 py-3.5 border-t border-[rgba(255,255,255,0.07)] flex items-center justify-end gap-2.5 shrink-0">
                <button
                  type="button"
                  onClick={() => setPaymentModalInvoice(null)}
                  className="h-10 px-4 rounded-xl bg-[var(--k-surface)] hover:bg-[var(--k-surface-raised)] text-[var(--k-text-secondary)] hover:text-white border border-[rgba(255,255,255,0.07)] text-xs font-mono font-medium transition-colors min-h-[40px]"
                >
