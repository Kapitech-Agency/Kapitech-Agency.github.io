  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[rgba(255,255,255,0.07)]">
        <div>
          <div className="flex items-center gap-2 text-[var(--k-red)] font-mono text-xs font-semibold uppercase tracking-wider mb-1">
            <Cpu size={14} />
          ...
          <p className="text-xs sm:text-sm text-[var(--k-text-secondary)] mt-1">
            Manage live service offerings, SLA benchmarks, deliverables, and technical capabilities published on <code className="text-white bg-[var(--k-surface-raised)] px-1.5 py-0.5 rounded border border-[rgba(255,255,255,0.07)]">/services</code>.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <div className="text-xs font-mono text-[var(--k-text-secondary)] bg-[var(--k-surface)] px-3.5 py-2 rounded-xl border border-[rgba(255,255,255,0.07)]">
            Active Catalog: <strong className="text-white">{servicesList.length} Offerings</strong>
          </div>

          <button
            onClick={() => setIsAddModalOpen(true)}
            className="px-4 py-2 rounded-xl bg-[var(--k-red)] hover:bg-[var(--k-red-hover)] text-white text-xs font-sans font-semibold transition-all flex items-center gap-1.5 shadow-lg shadow-[var(--k-red)]/25 min-h-[38px]"
          >
            <Plus size={14} />
      ...
      {statusMessage && (
        <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-sans flex items-center justify-between animate-in fade-in duration-200">
          ...
          <button onClick={() => setStatusMessage(null)} className="text-[var(--k-text-secondary)] hover:text-white">
            <X size={14} />
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
        <div className="w-full h-full bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] p-4 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            <div className="text-xs font-sans text-[var(--k-text-secondary)]">SEO & Growth Services</div>
        ...
        <div className="w-full h-full bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] p-4 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            <div className="text-xs font-sans text-[var(--k-text-secondary)]">Development & Cloud</div>
            <div className="text-xl font-sans font-bold text-white mt-0.5">5 Stacks</div>
          </div>
          <div className="w-8 h-8 rounded-lg bg-[var(--k-red)]/10 border border-[var(--k-red)]/30 flex items-center justify-center text-[var(--k-red)]">
            <FileCode2 size={16} />
          </div>
        </div>
        ...
        <div className="w-full h-full bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] p-4 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            <div className="text-xs font-sans text-[var(--k-text-secondary)]">UI/UX Design Systems</div>
        ...
        <div className="w-full h-full bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] p-4 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            <div className="text-xs font-sans text-[var(--k-text-secondary)]">Standard Delivery SLA</div>
      ...
      <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-3 bg-[var(--k-surface)] p-3 rounded-xl border border-[rgba(255,255,255,0.07)]">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-sans transition-all ${
                selectedCategory === cat
                  ? 'bg-[var(--k-red)] text-white font-semibold shadow-md shadow-[var(--k-red)]/20'
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] rounded-xl text-xs text-white placeholder:text-[#5C626E] focus:outline-none focus:border-[var(--k-red)] font-sans transition-colors min-h-[40px]"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 w-full">
        {filtered.map((item) => (
          <div
            key={item.slug}
            className="w-full h-full bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] rounded-xl p-5 flex flex-col justify-between hover:border-[rgba(255,255,255,0.14)] transition-all shadow-sm group"
          >
            <div>
              <div className="flex items-center justify-between gap-2 mb-3">
                <div className="flex items-center gap-2">
                  {getCategoryIcon(item.category)}
                  <span className="text-[10px] font-mono text-[var(--k-text-secondary)] uppercase tracking-wider font-semibold">
                    {item.category}
                  </span>
                </div>
              ...
              <h3 className="text-base font-bold font-sans text-white mb-1.5 group-hover:text-[var(--k-red)] transition-colors">
                {item.title}
              </h3>
              
              <p className="text-xs text-[var(--k-text-secondary)] leading-relaxed mb-4 line-clamp-2">
              ...
              <div className="space-y-1.5 mb-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {item.capabilities.slice(0, 3).map((c, idx) => (
                    <span key={idx} className="text-[10px] font-mono px-2 py-0.5 rounded-lg bg-[var(--k-surface-raised)] text-[var(--k-text)] border border-[rgba(255,255,255,0.07)]">
                      {c.title || c.titleId}
                    </span>
                  ))}
                </div>
              ...
              {item.tools && item.tools.length > 0 && (
                <div className="pt-2 border-t border-[rgba(255,255,255,0.07)] mb-3 flex items-center gap-1.5 overflow-x-auto custom-scrollbar pb-1">
                  ...
                  {item.tools.slice(0, 4).map((tool, tIdx) => (
                    <span key={tIdx} className="text-[9px] font-mono text-[var(--k-text-secondary)] bg-[var(--k-surface-raised)] px-1.5 py-0.5 rounded border border-[rgba(255,255,255,0.07)] shrink-0">
                      {tool}
                    </span>
                  ))}
                </div>
            ...
            <div className="pt-4 border-t border-[rgba(255,255,255,0.07)] flex items-center justify-between text-xs font-mono text-[var(--k-text-secondary)]">
              <span className="truncate max-w-[120px]">/{item.slug}</span>
              
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setSelectedServiceForDetail(item)}
                  className="px-2.5 py-1 rounded-lg bg-[var(--k-surface-raised)] hover:bg-[var(--k-surface-subtle)] border border-[rgba(255,255,255,0.07)] text-white text-[11px] font-sans flex items-center gap-1 transition-colors"
                >
      ...
      {selectedServiceForDetail && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] rounded-xl w-full max-w-2xl shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto custom-scrollbar animate-in zoom-in-95 duration-150">
            <button
              onClick={() => setSelectedServiceForDetail(null)}
              className="absolute top-4 right-4 p-1.5 rounded-lg text-[var(--k-text-secondary)] hover:text-white bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)]"
            >
              <X size={16} />
            </button>

            <div className="flex items-center gap-2 font-mono text-xs text-[var(--k-red)] font-semibold mb-2">
              <span>{selectedServiceForDetail.category}</span>
              <span>•</span>
              <span className="text-[var(--k-text-secondary)]">/{selectedServiceForDetail.slug}</span>
            </div>

            <h2 className="text-xl font-sans font-bold text-white mb-2">
              {selectedServiceForDetail.title}
            </h2>
            <p className="text-xs text-[var(--k-text-secondary)] mb-5 leading-relaxed">
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
              {selectedServiceForDetail.metrics?.map((m, idx) => (
                <div key={idx} className="bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] p-3 rounded-xl text-center">
                  <div className="text-lg font-sans font-bold text-[var(--k-red)]">{m.value}</div>
                  <div className="text-[10px] font-mono text-[var(--k-text-secondary)] mt-0.5">{m.label}</div>
                </div>
              ))}
            </div>
            ...
            <div className="mb-5">
              ...
              <div className="space-y-2">
                {selectedServiceForDetail.capabilities?.map((cap, cIdx) => (
                  <div key={cIdx} className="bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] p-3 rounded-xl">
              ...
              <div className="mb-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {selectedServiceForDetail.processStages.map((stage, sIdx) => (
                    <div key={sIdx} className="bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] p-3 rounded-xl">
                      <div className="flex items-center gap-2 text-[var(--k-red)] font-mono text-xs font-bold mb-1">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {stage.deliverables?.map((d, dIdx) => (
                          <span key={dIdx} className="text-[9px] font-mono bg-[var(--k-surface)] text-emerald-400 px-1.5 py-0.5 rounded border border-[rgba(255,255,255,0.07)]">
                            ✓ {d}
                          </span>
                        ))}
                      </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-[rgba(255,255,255,0.07)]">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setSelectedServiceForDetail(null)}
                  className="px-4 py-2 rounded-xl bg-[var(--k-surface-raised)] hover:bg-[var(--k-surface-subtle)] border border-[rgba(255,255,255,0.07)] text-white text-xs font-sans transition-colors"
                >
                  Close
                </button>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-4 py-2 rounded-xl bg-[var(--k-red)] hover:bg-[var(--k-red-hover)] text-white text-xs font-sans font-semibold flex items-center gap-1 shadow-md shadow-[var(--k-red)]/20 transition-all"
                >
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] rounded-xl w-full max-w-lg shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto custom-scrollbar animate-in zoom-in-95 duration-150">
            <button
            ...
            <p className="text-xs text-[var(--k-text-secondary)] mb-5">
            ...
            <form onSubmit={handleCreateService} className="space-y-4 text-xs font-sans">
              <div>
                <label className="block text-[var(--k-text-secondary)] mb-1 font-medium">Service Title *</label>
                <input
                  type="text"
                  required
                  value={newTitle}
                  ...
                  className="w-full px-3 py-2 rounded-xl bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] text-white focus:outline-none focus:border-[var(--k-red)]"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[var(--k-text-secondary)] mb-1 font-medium">URL Slug *</label>
                  <input
                    type="text"
                    required
                    value={newSlug}
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] text-white font-mono text-[11px] focus:outline-none focus:border-[var(--k-red)]"
                  />
                </div>
                <div>
                  <label className="block text-[var(--k-text-secondary)] mb-1 font-medium">Category</label>
                  <select
                    value={newCategory}
              ...
              <div>
                <label className="block text-[var(--k-text-secondary)] mb-1 font-medium">Hero Headline</label>
                <input
                  type="text"
                  value={newHeadline}
              ...
              <div>
                <label className="block text-[var(--k-text-secondary)] mb-1 font-medium">Description & Scope Overview</label>
                <textarea
                  rows={2}
                  value={newSubtitle}
              ...
              <div>
                <label className="block text-[var(--k-text-secondary)] mb-1 font-medium">Core Deliverables & Capabilities</label>
                <div className="flex gap-2 mb-2">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 rounded-xl bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] text-white text-xs focus:outline-none focus:border-[var(--k-red)]"
                  />
                  <button
                    type="button"
                    onClick={handleAddTempCapability}
                    className="px-3 py-1.5 rounded-xl bg-[var(--k-surface-raised)] hover:bg-[var(--k-surface-subtle)] border border-[rgba(255,255,255,0.07)] text-white text-xs font-semibold transition-colors"
                  >
                    Add
                  </button>
                </div>
                <div className="space-y-1.5 max-h-32 overflow-y-auto custom-scrollbar">
                  {tempCapabilities.map((cap, idx) => (
                    <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] text-[11px]">
              ...
              <div className="flex items-center justify-end gap-3 pt-3">
                <button
                  type="button"
                  onClick={() => setIsAddModalOpen(false)}
                  className="px-4 py-2 rounded-xl bg-[var(--k-surface-raised)] hover:bg-[var(--k-surface-subtle)] border border-[rgba(255,255,255,0.07)] text-white text-xs transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-[var(--k-red)] hover:bg-[var(--k-red-hover)] text-white text-xs font-semibold shadow-lg shadow-[var(--k-red)]/25 transition-all"
                >
                  Publish Service
                </button>
              </div>
