    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[rgba(255,255,255,0.07)]">
        <div>
          <h1 className="text-2xl font-display font-bold text-white flex items-center gap-3">
            <Quote className="text-[var(--k-red-hover)]" size={24} />
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-[var(--k-red)] hover:bg-[var(--k-red-hover)] text-white text-xs font-mono font-bold transition-all flex items-center gap-2 shadow-lg shadow-[var(--k-red)]/20 min-h-[44px]"
        >
          <Plus size={14} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {testimonials.map((item) => (
          <div
            key={item.id}
            className="bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-6 flex flex-col justify-between hover:border-[rgba(255,255,255,0.15)] transition-all"
          >
            <div>
              {/* Rating stars */}
            ...
            <div className="pt-4 border-t border-[rgba(255,255,255,0.07)] flex items-center justify-between">
              <div>
                ...
                <div className="text-[11px] font-mono text-[var(--k-text-secondary)]">
                  {item.role}, <span className="text-white">{item.company}</span>
                </div>
                <div className="text-[10px] font-mono text-[var(--k-text-muted)] mt-0.5 flex items-center gap-1">
                  <MapPin size={10} />
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="p-2 rounded-lg bg-[var(--k-surface-raised)] hover:bg-[var(--k-surface-subtle)] text-[var(--k-text-secondary)] hover:text-white border border-[rgba(255,255,255,0.07)] transition-colors min-h-[36px] min-w-[36px] flex items-center justify-center"
                >
                  <Edit3 size={14} />
                </button>
                <button
                  onClick={() => handleDelete(item.id, item.author)}
                  className="p-2 rounded-lg bg-[var(--k-surface-raised)] hover:bg-red-950/40 text-[var(--k-text-secondary)] hover:text-red-400 border border-[rgba(255,255,255,0.07)] hover:border-red-500/30 transition-colors min-h-[36px] min-w-[36px] flex items-center justify-center"
                >
                  <Trash2 size={14} />
                </button>
              </div>
      ...
      {isModalOpen && editingItem && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[var(--k-surface)] border border-[rgba(255,255,255,0.07)] rounded-2xl max-w-lg w-full p-6 sm:p-7 shadow-2xl">
            <div className="flex items-center justify-between pb-4 border-b border-[rgba(255,255,255,0.07)] mb-5">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-[var(--k-text-secondary)] hover:text-white text-xs font-mono">
            ...
            <form onSubmit={handleSaveModal} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] rounded-xl text-xs text-white focus:outline-none focus:border-[var(--k-red)] font-mono min-h-[44px]"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] rounded-xl text-xs text-white focus:outline-none focus:border-[var(--k-red)] font-mono min-h-[44px]"
                  />
                </div>
                <div>
                  <label className="block text-xs font-mono text-[var(--k-text-secondary)] mb-1 font-semibold">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 bg-[var(--k-surface-raised)] border border-[rgba(255,255,255,0.07)] rounded-xl text-xs text-white focus:outline-none focus:border-[var(--k-red)] font-sans"
                />
              </div>
              ...
              <div className="pt-2 flex items-center justify-end gap-2">
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-[var(--k-surface-raised)] text-[var(--k-text-secondary)] hover:text-white border border-[rgba(255,255,255,0.07)] text-xs font-mono min-h-[44px]"
                >
                  {language === 'id' ? 'Batal' : 'Cancel'}
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-[var(--k-red)] text-white text-xs font-mono font-bold hover:bg-[var(--k-red-hover)] transition-all shadow-md shadow-[var(--k-red)]/20 min-h-[44px]"
                >
