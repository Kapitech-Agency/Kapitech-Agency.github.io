...
test('task assignment uses relational assignee column',()=>{const s=read('server/postgres-task-repository.ts');assert.ok(s.includes('assignee_user_id'));assert.ok(s.includes('assignee: String(row.assignee_user_id'));});
