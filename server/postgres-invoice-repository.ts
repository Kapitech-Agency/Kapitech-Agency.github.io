...
export class PostgresInvoiceRepository {
  ...
  async update(id:string,patch:any):Promise<any|null>{return withPostgresTransaction(async db=>{const r=await db.query('SELECT * FROM invoices WHERE id=$1 FOR UPDATE',[id]);if(!r.rows[0])return null;const current=mapInvoice(r.rows[0],await this.items(db,id),await this.payments(db,id));if(current.status==='cancelled'&&patch.status&&patch.status!=='cancelled')throw new Error('Cancelled invoices cannot be reopened.');if(patch.updatedAt&&current.updatedAt!==patch.updatedAt)throw new Error('Invoice has been modified since it was loaded. Refresh and retry.');const next={...current,...patch,id,updatedAt:new Date().toISOString()};
      ...
      if (current.payments.length > 0 && (nextClientId !== String(current.clientId || '') || nextProjectId !== String(current.projectId || ''))) {
        throw new Error('PAID_INVOICE_LINKAGE_IMMUTABLE');
      }
      if (current.payments.length > 0 && Number(next.total) !== Number(current.total)) {
        throw new Error('PAID_INVOICE_TOTAL_IMMUTABLE');
      }
  ...
  async recordPayment(id:string,payment:any):Promise<any|null>{return withPostgresTransaction(async db=>{const r=await db.query('SELECT * FROM invoices WHERE id=$1 FOR UPDATE',[id]);if(!r.rows[0])return null;const current=mapInvoice(r.rows[0],await this.items(db,id),await this.payments(db,id));if(current.status==='cancelled')throw new Error('Cancelled invoices cannot receive payments.');if(Number(payment.amount)<=0||Number(payment.amount)>current.balanceDue)throw new Error('Payment exceeds the current invoice balance.');await db.query('INSERT INTO invoice_payments (id,invoice_id,amount,paid_at,method,reference,metadata) VALUES ($1,$2,$3,$4,$5,$6,$7)',[payment.id,id,payment.amount,payment.date,payment.method,payment.reference||null,JSON.stringify({recordedBy:payment.recordedBy||payment.userId||'system',notes:payment.notes||''})]);const totalPaid=current.amountPaid+Number(payment.amount),balance=Math.max(0,current.total-totalPaid),status=balance<=0?'paid':'partially_paid';const audit=[...current.auditTrail,{action:'payment_recorded',timestamp:new Date().toISOString(),user:payment.recordedBy||payment.userId||'system',note:payment.reference||payment.notes||''}];await db.query('UPDATE invoices SET amount_paid=$2,balance_due=$3,status=$4,metadata=$5,updated_at=$6 WHERE id=$1',[id,totalPaid,balance,status,JSON.stringify({...metadata(current),auditTrail:audit}),new Date().toISOString()]);return this.findByIdTx(db,id);});}
