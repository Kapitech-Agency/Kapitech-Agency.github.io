...
export class PostgresInvoiceRepository {
  ...
  async cancel(id:string,user:string):Promise<any|null>{return withPostgresTransaction(async db=>{const r=await db.query('SELECT * FROM invoices WHERE id=$1 FOR UPDATE',[id]);if(!r.rows[0])return null;const current=mapInvoice(r.rows[0],await this.items(db,id),await this.payments(db,id));if(current.status==='cancelled')return current;if(current.payments.length>0)throw new Error('INVOICE_WITH_PAYMENTS_CANNOT_BE_CANCELLED');const audit=[...current.auditTrail,{action:'cancelled',timestamp:new Date().toISOString(),user}];await db.query('UPDATE invoices SET status=$2,metadata=$3,updated_at=$4 WHERE id=$1',[id,'cancelled',JSON.stringify({...metadata(current),auditTrail:audit}),new Date().toISOString()]);return this.findByIdTx(db,id);});}
