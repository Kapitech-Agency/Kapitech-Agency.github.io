...
apiRouter.get('/crm/deals', requireAuth, requirePermission('canManageCrm'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const deals = getDataSourceMode() === 'postgres' ? await postgresCrmDealRepository.list() : (await loadApplicationDatabase()).crmDeals;
...
apiRouter.put('/crm/deals/:id', requireAuth, requirePermission('canManageCrm'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  if (patch.expectedCloseDate !== undefined && patch.expectedCloseDate !== '' && !isValidDate(patch.expectedCloseDate)) {
    res.status(400).json({ success: false, error: 'Invalid expected close date.' });
    return;
  }
...
apiRouter.delete('/crm/deals/:id', requireAuth, requirePermission('canManageCrm'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { id } = req.params;
  if (getDataSourceMode() === 'postgres') {
    const deleted = await postgresCrmDealRepository.delete(id);
    if (!deleted) {
      res.status(404).json({ success: false, error: 'Deal not found.' });
      return;
    }
  ...
  const db = getDatabase();
  const exists = db.crmDeals.some(d => d.id === id);
  if (!exists) {
    res.status(404).json({ success: false, error: 'Deal not found.' });
    return;
  }
