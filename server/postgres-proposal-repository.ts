...
if(p.clientId){const x=await db.query('SELECT id FROM clients WHERE id=$1 LIMIT 1',[p.clientId]);if(!x.rows[0])throw new Error('Proposal client not found.');}
if(p.projectId){const x=await db.query('SELECT id, client_id FROM projects WHERE id=$1 LIMIT 1',[p.projectId]);if(!x.rows[0])throw new Error('Proposal project not found.');if(p.clientId&&x.rows[0].client_id&&String(x.rows[0].client_id)!==String(p.clientId))throw new Error('Proposal project does not belong to the selected client.');}
if(p.dealId){const x=await db.query('SELECT id, client_id FROM crm_deals WHERE id=$1 LIMIT 1',[p.dealId]);if(!x.rows[0])throw new Error('Proposal deal not found.');if(p.clientId&&x.rows[0].client_id&&String(x.rows[0].client_id)!==String(p.clientId))throw new Error('Proposal deal does not belong to the selected client.');}
