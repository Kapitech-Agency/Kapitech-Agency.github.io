  ...
  async convertLead(lead: any, client: any, deal: any, clientAlreadyExists = false): Promise<{ client: any; deal: any }> {
    return withPostgresTransaction(async db => {
      ...
      if (clientAlreadyExists) {
        const existing = await db.query('SELECT id FROM clients WHERE id=$1 LIMIT 1',[client.id]);
      ...
      const leadResult = await db.query('UPDATE leads SET status=$2,updated_at=$3 WHERE id=$1 AND status <> $4 RETURNING *', [lead.id, 'closed', deal.createdAt, 'closed']);
