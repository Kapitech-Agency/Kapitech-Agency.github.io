-- AMS PostgreSQL migration 028
-- Repair deferred financial reconciliation trigger row handling.
-- Migration 027 is already applied on existing databases, so the trigger
-- function definitions must be replaced by a new forward-only migration.
